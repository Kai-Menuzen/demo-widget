...
!(function (e, t) {
      ...
      ? ((e._handled = !0),
        t._immediateFn(function () {
          ...
          if (null !== t) {
            ...
            o(n.promise, i);
          } else (1 === e._state ? o : r)(n.promise, e._value);
        }))
      : e._deferreds.push(n);
  }
  function o(e, n) {
  ...
  function i(e) {
    ...
    for (var o = 0, r = e._deferreds.length; r > o; o++) n(e, e._deferreds[o]);
    e._deferreds = null;
  }
  function u(e, t) {
    var n = !1;
  ...
  return (
    ...
    (t.all = function (e) {
      return new t(function (t, n) {
        ...
        for (var i = r.length, u = 0; r.length > u; u++) o(u, r[u]);
      });
    }),
...
function Signagelive() {
  try {
    ...
    self.onRequestMediaPlayerMoveToNextAssetError = function (
      messageId,
      error
    ) {
      self.messages[messageId].reject(new Error(error.reason));
    ...
    self.getExternalData = function (
      ...
      var uniqueIntegrationId = getUniqueIntegrationId(
        dataSource,
        integrationId,
        integrationName
      );
      self.onExternalDataUpdates[uniqueIntegrationId] = onDataUpdate;
      // Send message
      var message = {
        method: 'getExternalData',
        params: [dataSource, integrationId, integrationName],
        successCallback: 'onGetExternalDataSuccess',
        errorCallback: 'onGetExternalDataError',
      };
      return sendMessage(message);
    };
    self.onGetExternalDataSuccess = function (messageId, data) {
      self.messages[messageId].resolve(data);
    };
    self.onGetExternalDataError = function (messageId, error) {
      self.messages[messageId].reject(new Error(error.reason));
    };
    self.onExternalDataUpdate = function (messageId, response) {
      // Get update details
      var dataSource = response.dataSource;
      var integrationName = response.integrationName;
      var integrationId = response.integrationId;
      var data = response.data;
      // Get unique integration id
      var uniqueIntegrationId = getUniqueIntegrationId(
        dataSource,
        integrationId,
        integrationName
      );
      // Call correct callback with data
      self.onExternalDataUpdates[uniqueIntegrationId](data);
    };

    function getUniqueIntegrationId(
      dataSource,
      integrationId,
      integrationName
    ) {
    ...
    self.playVideo = function (
      ...
      var mockPlayVideoAPI =
        (navigator.userAgent.indexOf('Tizen') != -1) === false;
      if (mockPlayVideoAPI) {
        return mockPlayVideo(
          uris,
          x,
          y,
          width,
    ...
    self.onVideoTimeUpdate = function (messageId, data) {
      if (self.videoCallbacks && self.videoCallbacks.onTimeUpdate) {
        if ((navigator.userAgent.indexOf('Tizen') != -1) === false) {
    ...
    function mockPlayVideo(uris, x, y, width, height, loop) {
      ...
      isVideoPlaylist = Array.isArray(uris);
      videoPlaylist = uris;
      playlistPosition = 0;
      // Return a promise
      if (isVideoPlaylist) {
        return new Promise(function (resolve, reject) {
          Signagelive.log('Playing video Playlist');
          ...
          video1.style.top = positionY + 'px';
          video1.style.left = positionX + 'px';
          video1.style.width = mockWidth + 'px';
          video1.style.height = mockHeight + 'px';
          Signagelive.log('Video1 visible');
    ...
    function updateVideoPlayingIndex() {
      Signagelive.log('update video playing index');
    ...
    function onMockedVideoTimeUpdate(event) {
      ...
      if (currentVideoSrc !== '') {
        if (
          (isVideoPlaylist &&
            currentVideoSrc.indexOf(videoPlaylist[playlistPosition]) == -1) ||
          !isVideoPlaylist
        ) {
          if ((navigator.userAgent.indexOf('Tizen') != -1) === false) {
    ...
    function onMockedVideo1Playing(event) {
      ...
      Signagelive.log('Video1 playing');
      var video1 = document.getElementById('sl_video_1');
      var video2 = document.getElementById('sl_video_2');
      ...
      setTimeout(function () {
        if (video2) {
          Signagelive.log('Video2 removed');
          video2.style.visibility = 'hidden';
          video2.remove();
        }
      }, 0);

      setDurationForVideoRemoval(video1.duration, 3000);

      var video2 = document.getElementById('sl_video_2');
      setTimeout(function () {
        if (!video2 && isVideoPlaylist) {
          // Create 1st video element
          Signagelive.log('Create video2');
          var video2 = document.createElement('video');
          video2.id = 'sl_video_2';
          video2.style.position = 'absolute';
          video2.style.top = positionY + 'px';
          video2.style.left = positionX + 'px';
          video2.style.width = mockWidth + 'px';
          video2.style.height = mockHeight + 'px';
          video2.style.visibility = 'hidden';
    ...
    function onMockedVideo2Playing(event) {
      self.messages['mockVideoPlaying'](true);
      // Show video 1 and hide video2
      Signagelive.log('Video2 playing');
      var video1 = document.getElementById('sl_video_1');
      var video2 = document.getElementById('sl_video_2');

      if (video2) {
        Signagelive.log('Video2 visible');
        video2.style.visibility = 'visible';
      }

      setTimeout(function () {
        if (video1) {
          Signagelive.log('Video1 removed');
          video1.style.visibility = 'hidden';
          video1.remove();
        }
      }, 0);

      setDurationForVideoRemoval(video2.duration, 3000);

      var video2 = document.getElementById('sl_video_2');
      setTimeout(function () {
        if (!video1 && isVideoPlaylist) {
          // Create 1st video element
          Signagelive.log('Create video1');
          var video1 = document.createElement('video');
          video1.id = 'sl_video_1';
          video1.style.position = 'absolute';
          video1.style.top = positionY + 'px';
    ...
    function setDurationForVideoRemoval(videoDuration, timeAmendment) {
      if ((navigator.userAgent.indexOf('Tizen') != -1) === false) {
        duration = Math.round(Number(videoDuration)) * 1000 - timeAmendment;
      } else {
        duration = Math.round(Number(videoDuration) - timeAmendment);
      }
    }
    ...
    function onMockedVideo1Ended(event) {
      ...
      // pause current video
      var video1 = document.getElementById('sl_video_1');
      Signagelive.log('Video1 not paused');
      video1.pause();
      // Move next
      var video2 = document.getElementById('sl_video_2');
      if (video2) {
        Signagelive.log('play video2');
        video2.play();
      }
      // else {
      //     setTimeout(function() {
      //         Signagelive.log('video wasnt created - try again');
      //         video2.play();
      //     }, 0);
      // }
    }
    ...
    function onMockedVideo2Ended(event) {
      ...
      // pause current video
      var video2 = document.getElementById('sl_video_2');
      Signagelive.log('Video2 not paused');
      video2.pause();
      // Move next
      var video1 = document.getElementById('sl_video_1');
      if (video1) {
        Signagelive.log('play video1');
        video1.play();
      }
      // else {
      //     setTimeout(function() {
